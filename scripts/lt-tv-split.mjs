...
async function main() {
  ...
  // This is the end of the chain, so it has to hand over the last step. Without
  // it the guide still says "Not recorded yet" and the obvious-looking way to
  // fix that is to re-run the generator, which rebuilds the episode from
  // scratch and throws away both the edits and the timing just recorded.
  console.log(`\nOnce they are uploaded, put the episode on the guide:`);
  console.log(`  npm run lt:slate -- ${id}`);
  console.log("Do NOT re-run the generator for this — it would rebuild the episode and");
  console.log("discard your edits and the timing you just recorded.");
}
