...
// this too.
export const FACE_BEAT_RULES = `FACE BEATS: the same "cues" array also takes facial expressions, which SitePal performs on that character's own face: ${FACE_NAMES.join(", ")}. A face beat is written exactly like a body beat — { "actor", "reaction": "smile", "offset" } — and every character has every face. Use one where the face says something the words do not: the speaker's smile on a line they are enjoying, the listener's disgusted look at a bad trade, thinking just before a considered answer, surprise at a number. Aim for roughly one face beat every four or five lines across the episode, no more than one per line, and never the same face on the same character twice running. A face holds about three seconds and relaxes by itself. Wink at most once an episode, on a line that is in on its own joke. Faces follow character: Connor's are knowing — a smile that is a smirk, surprise that is mock innocence, a wink — and never sadness or fear in earnest. These are drafts: the producer reads the script through and deletes any that land wrong, so place each one where you mean it rather than to fill a quota.`;
