...
export function renderScript(episode) {
  ...
  const out = [
    ...
    "#",
    "# Two marks change the recording without changing a word. Put either on",
    ...
    "# A pause is real silence, recorded in, and it is the same length every",
    "# time — ElevenLabs cannot do that itself. It also ends the recording",
    "# there and starts a new one, so the delivery either side of a pause can",
    "# shift a little. For a beat that only needs to sound right rather than be",
    "# a measured length, [long pause] inside the line is cheaper.",
    "# Applying edits rewrites this file and wipes both marks, so mark them",
    "# once the words are settled.",
    "#",
