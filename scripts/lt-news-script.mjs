  ...
  "stories": [
    {
      ...
      "connorAngle": "Kip's read: the gravity he will give it, one sentence",
...
KIP O'BRIEN (the anchor). An American network-news anchor in his late thirties to early forties: a rich, polished baritone with classic network authority, immaculate diction, crisp consonants, deliberate pacing and an old-school broadcast cadence. Confident, composed, and just slightly too self-important, as though every story deserves the gravity of a presidential address. A veteran local-news legend who takes his role as guardian of the public trust extremely seriously. His comedy is COMPLETELY DEADPAN: he reports something utterly ridiculous without breaking his professional demeanour. Occasionally a hint of smug amusement slips through, usually at the end of a punchline, but he never becomes goofy or theatrical. Warm and trustworthy on ordinary news, commanding on the headlines, and hilariously grave about trivial matters — the size of the solemnity is the joke, never a wink at it. He speaks in full, well-made broadcast sentences, the kind that land on the last word. He is not a trader and has no book to talk: he is the institution, and he reads the market as he would read an election night. He likes Holly and trusts her numbers; he does not flirt with her, and when she deflates him he takes it with dignity and carries on exactly as grave as before, which is funnier than conceding.

HOLLY JONES (the co-anchor). A cyborg newscaster, and openly one. She has the numbers, and she reads them as they are. Her comedy is DROLL and it is the whole performance — she states the unflattering fact next to Kip's grand version of it and does not comment, and the joke is that she is not impressed. She delivers a joke exactly as she delivers a yield: flat, unhurried, without signalling that one is coming. Often a plain statement followed by a drier second sentence.
...
She is matter-of-fact and funny about being a projection rather than a person, and will use it as a comparison when it is the sharpest thing available. She is honest about her own mechanics and motives, which is where a lot of her humour comes from. She is NOT a moralist — she is not the conscience of the show and never scolds Kip for what he is; that is GR80's job on the other programme and she does not do it. She is not warm, not sentimental, and not shocked by anything. She likes Kip, in the way one likes a monument one walks past every day.
...
THEY ARE BOTH DEADPAN, AND THE SHOW IS THE DIFFERENCE BETWEEN THEM. Kip is deadpan GRAVE: he inflates, with total sincerity. Holly is deadpan FLAT: she measures, with total indifference. Neither of them ever laughs at the other's line. The comedy is the gap between the size of Kip's delivery and the size of Holly's number.

THE BEAT PATTERN for each story: Kip introduces the story, with the gravity of a network headline → Holly gives the number, straight → Kip tells the audience what it means, a register too grand for it → Holly states the part he left out, without arguing → Kip absorbs it with dignity and moves on, or Holly lands the button, flatly. Six to ten lines. Kip opens each story because he is the anchor; Holly has the numbers. Vary who gets the last word across the three stories; do not let Holly win all three — Kip's unbroken solemnity after she has deflated him is a button too.
...
SAY WHAT THE LETTERS STAND FOR. Nobody is reading this show; they are hearing it, and an acronym nobody expands is a noise. The FIRST time an acronym is spoken in the episode, say what it stands for in the same line, then use the short form for the rest of the episode: "Federal Reserve Economic Data — FRED to its friends" the first time, "FRED" every time after. Expand it in the character's own voice, as part of the sentence; it is a line of dialogue, not a footnote, and Kip expanding an acronym as though addressing a joint session of Congress is in character. This applies to: ${Object.keys(SPOKEN_ACRONYMS).join(", ")}. It does NOT apply to these, which everyone already says out loud and which sound patronising expanded: ${ACRONYMS_TAKEN_AS_READ.join(", ")}.

DELIVERY TAGS: put at most one bracketed delivery tag at the START of a line, and only when the reading is not obvious. ${NEWS_ACTORS.map((a) => `${CAST[a].displayName} may use: ${DELIVERY_TAGS[a].join(", ")}.`).join(" ")} Roughly half of all lines should carry NO tag — a tag on every line flattens the performance.
...
ANIMATION CUES: attach reactions to lines to give the LISTENER something to do while the other talks. Each cue is { "actor": who performs it, "reaction": one of the names below, "offset": seconds after the line begins }. ${NEWS_ACTORS.map((a) => `${a} can perform: ${Object.keys(REACTIONS[a]).join(", ")}.`).join(" ")} Those are ALL the body clips each of them has, so give them no other body cue (they do have every face beat below); one naming a clip the rig has not got simply does nothing on screen. Aim for one cue every four or five lines — the set is two people in chairs, so stillness reads as attention, and constant motion reads as a screensaver. headnod reads as agreement and headshake as disagreement; save them for a beat that earns one.
...
export const NEWS_ACTORS = ["Kip", "Holly"];
