...
// Stores features/trading patterns, lessons learned, and performance metrics
...
class KnowledgeBase {
  constructor() {
    ...
    this.features/tradingPatternsCol = collection(this.db, 'knowledge_patterns');
  ...
  async recordTradingPattern(pattern) {
    ...
    // Store in Firestore
    await setDoc(doc(this.features/tradingPatternsCol, patternDoc.id), patternDoc);
  ...
  async findSimilarPatterns(pattern, threshold = 0.8) {
    try {
      ...
      const q = query(
        this.features/tradingPatternsCol,
  ...
  async integrateLessonIntoStrategy(lesson) {
    // Create or update features/trading rule based on lesson
  ...
  async getRelevantKnowledge(context) {
    ...
    // Get active features/trading rules
  ...
  async addToTradingRules(rule, confidence = null) {
    const rulesDoc = doc(this.db, 'features/trading_rules', rule.id || `rule_${Date.now()}`);
  ...
  async getActiveTradingRules() {
    try {
      ...
      const rulesQuery = query(
        collection(this.db, 'features/trading_rules'),
