// Simplified client-side hook for Lighter features/trading via API route
...
export function useLighterAPI(config = {}) {
  ...
  const fetchAccountState = useCallback(async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        if (false) {
        try {
          // Main features/trading analysis
          const features/tradingPrompt = `Market snapshot: BTC $${marketContext.btcPrice || 'N/A'} (${marketContext.btcChange}% 24h), ETH $${marketContext.ethPrice || 'N/A'} (${marketContext.ethChange}% 24h). ${marketContext.positionCount} positions, P&L: $${marketContext.totalPnL.toFixed(2)}. What's the play?`;
          
          const features/tradingResponse = await fetch('/api/ai/analyze', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prompt: features/tradingPrompt, consultant: 'features/trading', marketData: marketContext })
          });
          
          if (features/tradingResponse.ok) {
            const { analysis } = await features/tradingResponse.json();
            // Store as strategy insight for the main display
            setAssistantInsights(prev => ({ ...prev, strategy: analysis }));
            newThoughts.push({
              timestamp: new Date().toLocaleString(),
              type: 'features/trading',
              message: analysis,
              consultant: 'market'
            });
          }
  ...
  const formatTradingData = useCallback(() => {
    ...
    return {
      ...
      totalPnl: 0, // No P&L until we start features/trading
      totalPnlPercent: 0, // No returns yet
      
      // Performance Metrics (all 0 until we start features/trading)
