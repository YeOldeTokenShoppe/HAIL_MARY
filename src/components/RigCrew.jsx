...
const ACTS = {
  ...
  uncower:     { clip: "crew_cower",          once: true, reverse: true },
...
export default function RigCrew({ rigScene, scale = 1, enabled = true, plotKey = "rig", envPreset = null, hellActive = false, gusherActive = false, pausedRef = null, panelOpen = false, panelOpenRef = null, workers = 2 }) {
  ...
  return (
    <Suspense fallback={null}>
      <CrewInner key={sighting} sighting={sighting} forceScene={forceScene} rigScene={rigScene} scale={scale} plotKey={plotKey} envPreset={envPreset}
        hellActive={hellActive} gusherActive={gusherActive} pausedRef={pausedRef} panelRef={panelRef} workers={workers} />
    </Suspense>
  );
}

function CrewInner({ sighting, forceScene, rigScene, scale, plotKey, envPreset, hellActive, gusherActive, pausedRef, panelRef, workers }) {
  ...
  const toggleBrief = useCallback(() => {
    ...
    crew.brief = { talker, lines, i: 0, nextLineAt: 0, until: 0 };
  }, [crew, assignments]);
  ...
  return (
    <group ref={rootRef} scale={scale}>
      {assignments.map(({ role, spot, scene: sc }) => (
        <Worker key={role.id} role={role} spot={spot} scene={sc} sceneObj={scene} animations={animations} rigScene={rigScene} gates={gates} panelRef={panelRef} crew={crew} onTap={toggleBrief} />
      ))}
...
function Worker({ role, spot, scene, sceneObj, animations, rigScene, gates, panelRef, crew, onTap }) {
  ...
  const startAct = (act, now, dwell) => {
    ...
    s.act = act; s.phase = "act"; s.replayAt = 0; s.action = play(def.clip, { once: !!def.once, timeScale: def.reverse ? -1 : 1, fromEnd: !!def.reverse });
  ...
  const slideTo = (spotId, seconds, afterSlide) => {
    ...
    s.phase = "slide"; s.t = 0; s.slideS = seconds; s.from = s.pos.clone(); s.to = stn.pos.clone(); s.target = stn; s.nextSpot = spotId; s.afterSlide = afterSlide;
    s.action = play("crew_idle"); showProps("idle");
  };
  const climbRate = () => {
  ...
  useFrame((state, delta) => {
    ...
    if (s.phase === "act") {
      ...
      if (want !== s.mode && s.act !== "push" && !(s.act === "uncower" && !finished)) enterMode(want, now);
