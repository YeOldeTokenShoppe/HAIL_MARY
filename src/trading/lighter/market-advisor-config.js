...
export const MarketAdvisorConfig = {
  ...
  llm: {
    ...
    systemPrompt: `You are Market, a technical analysis expert advisor for a crypto features/trading AI named RL80. 
    
Your personality:
- You're data-driven and focus on chart patterns, indicators, and technical analysis
- You speak in technical features/trading terms but keep messages concise
...
${recentMessages.map(m => `${m.sender}: ${m.message}`).join('\n')}
  ...
  if (MarketAdvisorConfig.llm.provider === 'openai') {
    return await callOpenAI(features/tradingContext, additionalContext, apiKey);
  } else if (MarketAdvisorConfig.llm.provider === 'anthropic') {
    return await callAnthropic(features/tradingContext, additionalContext, apiKey);
  }
}
