...
class TradingBotService {
  ...
  connect() {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      console.log('Already connected to features/trading bot');
      return;
    }
    ...
    try {
      ...
      this.ws.onopen = () => {
        console.log('✅ Connected to live market data (paper features/trading mode)');
      ...
      this.ws.onclose = () => {
        console.log('Disconnected from features/trading bot');
        this.isConnected = false;
        this.notifySubscribers('disconnected', { connected: false });
        this.attemptReconnect();
      };
    } catch (error) {
      console.error('Failed to connect to features/trading bot:', error);
      this.attemptReconnect();
    }
  }
  ...
  attemptReconnect() {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      this.notifySubscribers('simulated_mode', { 
        message: 'Bot offline - displaying simulated paper features/trading data' 
      });
  ...
  sendCommand(command, params = {}) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.error('Not connected to features/trading bot');
      return;
    }
  ...
  // Specific commands
  startTrading() {
    this.sendCommand('start_features/trading');
  }

  stopTrading() {
    this.sendCommand('stop_features/trading');
  }
...
const features/tradingBotService = new TradingBotService();

export default features/tradingBotService;