...
| Type | Count | Notes |
|---|---|---|
| Traders | 4 | The cast: Eugene, Marisol, Saint GR80, Connor. All Mythic. Playable characters, never in the draw pool. **Cast reconciliation applied 2026-07-14** — `halo-node` → `gr80`, `bullhorn-broker` → `john-barron`, Eugene recharacterized as the Pattern Prophet ("Déjà Vu" ability), `automation` tag → `discipline`, new `pattern` tag, meme lane moved to Connor; see [CASE_TABLE.md](./CASE_TABLE.md) §2. |
