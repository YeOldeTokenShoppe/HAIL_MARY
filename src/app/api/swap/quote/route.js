...
const CDP_PATH = '/platform/v2/evm/swaps/quote';
...
export async function POST(request) {
  ...
  try {
    ...
    if (Number.isInteger(slippageBps)) {
      query.set('slippageBps', String(slippageBps));
    }
    ...
    const cdpRes = await fetch(`https://${CDP_HOST}${CDP_PATH}?${query.toString()}`, {
      method: 'GET',
      headers: {
    ...
    if (!cdpRes.ok) {
      console.error('CDP swap quote error:', cdpRes.status, text);
      return NextResponse.json(
        { error: 'Quote unavailable', detail: text },
