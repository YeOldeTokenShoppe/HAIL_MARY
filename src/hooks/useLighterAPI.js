...
export function useLighterAPI(config = {}) {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      // DISABLED: Frontend chat generation
      // Chats are now generated by the hourly cron job (/api/cron/run-scoring)
      // which runs EMO → TEKNO → MACRO → RL80 in sequence and saves to agentChat
      // The frontend just displays messages from Firestore via FocusedAgentCard

      // If you need manual testing, uncomment below:
      // const testDelay = setTimeout(() => generateTeamChat(), 5000);
